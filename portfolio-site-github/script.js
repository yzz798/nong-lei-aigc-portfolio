...
function mediaMarkup(project, mode = "preview") {
  if (project.video) {
    ...
    return `<video src="${project.video}" ${controls} playsinline preload="metadata" data-start="${start}" data-end="${end}" data-loop-clip="${loopClip}"></video>`;
  }
...
function openProject(projectId) {
  ...
  const actionLinks = [
    ...(project.link ? [{ label: "打开 Demo", href: project.link }] : []),
    ...(project.links || []),
  ];
